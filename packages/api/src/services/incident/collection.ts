 ...
 * 注册触发器
 */
export async function registerTrigger(trigger: Trigger): Promise<void> {
...
export async function evaluateTriggers(
  ...
  for (const trigger of triggersStore.values()) {
    ...
    const matchedMetrics = metrics.filter(
      (m) => m.metric_name === trigger.condition.metric_name
    );
    ...
    for (const metric of matchedMetrics) {
      ...
      const threshold = trigger.condition.threshold as number;
    ...
    if (triggered) {
      ...
      console.log(
        `[Collection] Trigger ${trigger.trigger_id} fired with value ${matchedValue}`
      );
    }
  }
 ...
 * 匹配日志模式
 */
export async function matchLogPatterns(
  scope: IncidentScope,
  pattern: string
): Promise<LogEntry[]> {
 ...
 * 获取指标快照
 */
export async function getMetricSnapshot(scope: IncidentScope): Promise<MetricSnapshot> {
