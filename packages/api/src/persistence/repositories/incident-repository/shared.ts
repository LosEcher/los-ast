...
export const incidentMigrations: SqliteMigration[] = [
  {
    version: 1,
...
export function sortIncidentsByCreatedAtDesc(items: Incident[]): Incident[] {
  return [...items].sort(
    (left, right) => new Date(right.created_at).getTime() - new Date(left.created_at).getTime()
  );
}

export function paginateIncidentItems(
  items: Incident[],
  params: IncidentQueryParams
): IncidentListResponse {
...
export function queryIncidentItems(
  items: Incident[],
  params: IncidentQueryParams
): IncidentListResponse {
...
export function buildIncidentWhereClause(params: IncidentQueryParams): {
  whereClause: string;
...
export function buildIncidentScopedStats(
  items: Incident[],
...
export function buildIncidentStatusCounts(
  rows: Array<{ status: IncidentStatus; count: number | bigint }>
): {
  count: number;
  byStatus: Record<string, number>;
} {
...
export function parseStoredIncident(
