...
export function assertApprovalScope(params: ApprovalQueryParams): asserts params is ApprovalQueryParams & {
  tenant_id: string;
  project_id: string;
} {
...
export function filterApprovalsByQuery(items: ApprovalItem[], params: ApprovalQueryParams): ApprovalItem[] {
...
export function sortApprovalsByCreatedAtDesc(items: ApprovalItem[]): ApprovalItem[] {
  return [...items].sort(
    (left, right) => new Date(right.created_at).getTime() - new Date(left.created_at).getTime()
  );
}

export function paginateApprovalItems(items: ApprovalItem[], params: ApprovalQueryParams): ApprovalQueryResult {
...
export function queryApprovalItems(items: ApprovalItem[], params: ApprovalQueryParams): ApprovalQueryResult {
  return paginateApprovalItems(sortApprovalsByCreatedAtDesc(filterApprovalsByQuery(items, params)), params);
}

export function createBaseApprovalStats(): ApprovalStats {
  return {
    total: 0,
    by_status: {
      pending: 0,
...
export function listExpiredPendingApprovalEntries(
  entries: Array<[string, ApprovalItem]>,
  referenceTime: string
): Array<[string, ApprovalItem]> {
  return entries.filter(([, approval]) =>
    approval.status === 'pending' && approval.timeout_at < referenceTime
  );
}

export function parseStoredApproval(rawValue: string | undefined, approvalId: string): ApprovalItem | undefined {
  if (!rawValue) {
    return undefined;
  }
...
export function buildApprovalWhereClause(params: ApprovalQueryParams): {
  whereClause: string;
