...
function extractAuthHeader(request: FastifyRequest): string | null {
...
function assertClientScopeNotTampered(
  clientScope: Scope,
  verified: { tenant_id: string; project_id: string; actor_id: string }
): void {
  ...
  if (mismatches.length > 0) {
    throw new ScopeError(
      'SCOPE_TAMPERED',
      `Client scope does not match verified identity claims: ${mismatches.join(', ')}`
    );
  }
}
...
async function verifyIdentity(
  ...
  if (IS_PRODUCTION && JWT_CONFIG.enforceJWT) {
    if (!token) {
      throw new AuthenticationError(
        'MISSING_JWT',
        'JWT token is required in production environment'
      );
    }
    ...
    if (!payload) {
      throw new AuthenticationError(
        'INVALID_JWT_PAYLOAD',
        'JWT payload is missing or invalid'
      );
    }
    ...
    if (!actorId) {
      throw new AuthenticationError(
        'MISSING_JWT_SUB',
        'JWT missing required "sub" (subject/actor) claim'
      );
    }
    ...
    if (!tenantId || !projectId) {
      ...
      throw new ScopeError(
        'INCOMPLETE_SCOPE',
        `JWT missing required scope claims: ${missing}`
      );
    }
  ...
  if (!IS_PRODUCTION) {
    if (!DEV_ALLOW_UNVERIFIED_IDENTITY) {
      throw new AuthenticationError(
        'UNVERIFIED_IDENTITY_DISABLED',
        'Unverified identity is disabled in development environment. Set DEV_ALLOW_UNVERIFIED_IDENTITY=true to allow local scope-based identity.'
      );
    }
...
export default fp(async function identityPlugin(fastify: FastifyInstance) {
  fastify.addHook('preHandler', async (request: FastifyRequest) => {
    ...
    if (!request.scope) {
      throw new ScopeError(
        'SCOPE_NOT_VALIDATED',
        'Identity validation requires scope to be validated first'
      );
    }
