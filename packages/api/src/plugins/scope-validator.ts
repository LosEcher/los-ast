...
import type { Scope } from '@los-ast/shared/types';

/**
 * Scope 验证插件
 * 硬约束 #3: 验证 scope 的完整性和环境适配性
 */
export default fp(async function scopeValidatorPlugin(fastify: FastifyInstance) {
  ...
  fastify.addHook('onRequest', async (request: FastifyRequest) => {
    ...
    if (SCOPE_CONFIG.requireFullScope) {
      ...
      if (!scope.tenant_id || !scope.project_id || !scope.actor_id) {
        const missing = [];
        if (!scope.tenant_id) missing.push('tenant_id');
        if (!scope.project_id) missing.push('project_id');
        if (!scope.actor_id) missing.push('actor_id');
        
        throw new ScopeError(
          'INCOMPLETE_SCOPE',
          `Incomplete scope in production environment. Missing: ${missing.join(', ')}`
        );
      }
 ...
 */
function extractScope(request: FastifyRequest): Scope | null {
  // 尝试从 body 获取 (POST/PUT 请求)
  if (request.body && typeof request.body === 'object') {
    const body = request.body as Record<string, unknown>;
    if (body.scope && typeof body.scope === 'object') {
      return body.scope as Scope;
    }
  }
  ...
  if (request.query && typeof request.query === 'object') {
    ...
    if (query.scope) {
      if (typeof query.scope === 'string') {
        try {
          return JSON.parse(query.scope) as Scope;
        } catch {
          return null;
        }
      }
      if (typeof query.scope === 'object') {
        return query.scope as Scope;
      }
    }
  }
