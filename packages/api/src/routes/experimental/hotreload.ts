...
import type {
  ...
  ValidateConfigBundleRequest,
  ActivateConfigBundleRequest,
...
export default async function hotReloadRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/bundles/:id/validate', {
    ...
    const { id } = request.params as { id: string };
    const { validator_id } = request.body as ValidateConfigBundleRequest;
    const validatedBy = request.scope?.actor_id || validator_id;
    if (!validatedBy) {
      throw new ValidationError(
        'MISSING_ACTOR_ID',
        'Bundle validation requires a verified actor_id'
      );
    }
  ...
  fastify.post('/bundles/:id/activate', {
    ...
    const { id } = request.params as { id: string };
    request.body as ActivateConfigBundleRequest | undefined;
