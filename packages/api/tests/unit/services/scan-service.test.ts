...
describe('ScanService', () => {
  ...
  describe('execute', () => {
    ...
    it('should compare discriminator mapping additions and target changes in openApiComparisons', async () => {
      vi.mocked(core.scan).mockResolvedValue({
        filesScanned: 1,
        findings: [],
      } as any);
      ...
      const result = await scanService.execute({
        ...
        openApiComparisons: [
          {
            source: 'openapi-compare-discriminator-add-change',
            file: '/tmp/openapi-discriminator-add-change.yaml',
            format: 'yaml',
            baseline: [
              'openapi: 3.0.3',
              'paths:',
            ...
            current: [
              ...
              '                propertyName: kind',
              '                mapping:',
              "                  admin: '#/components/schemas/AdminUserV2'",
              "                  guest: '#/components/schemas/GuestUser'",
              "                  superadmin: '#/components/schemas/SuperAdminUser'",
              '      responses:',
              "        '200':",
              '          description: ok',
              '          content:',
              ...
              '                  propertyName: status',
              '                  mapping:',
              "                    active: '#/components/schemas/ActiveUserV2'",
              "                    disabled: '#/components/schemas/DisabledUser'",
              "                    pending: '#/components/schemas/PendingUser'",
            ].join('\n'),
          },
        ],
        signal: new AbortController().signal,
      ...
      expect(result.findings[0].excerpt).toContain('added superadmin');
      expect(result.findings[1].excerpt).toContain('admin: #/components/schemas/AdminUser -> #/components/schemas/AdminUserV2');
      expect(result.findings[2].excerpt).toContain('added pending');
      expect(result.findings[3].excerpt).toContain('active: #/components/schemas/ActiveUser -> #/components/schemas/ActiveUserV2');
    });
    ...
    it('should compare allOf-wrapped discriminator unions in openApiComparisons', async () => {
      vi.mocked(core.scan).mockResolvedValue({
        filesScanned: 1,
        findings: [],
      } as any);
    ...
    it('should treat serial-backed sql aliases as compatible with explicit nextval defaults', async () => {
      vi.mocked(core.scan).mockResolvedValue({
        filesScanned: 1,
        findings: [],
      } as any);
    ...
    it('should treat conservative postgres numeric aliases as compatible in schemaComparisons', async () => {
      vi.mocked(core.scan).mockResolvedValue({
        filesScanned: 1,
        findings: [],
      } as any);
