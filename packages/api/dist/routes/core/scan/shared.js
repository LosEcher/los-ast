...
export function resolveRulePackPatterns(rulePack, rulesRoot = resolveRulesRoot()) {
    if (!rulePack) {
        return undefined;
    }
...
export function hasNativeArtifactInputs(body) {
    return SCAN_NATIVE_INPUT_KEYS.some((key) => {
        const items = body[key];
        return Array.isArray(items) && items.length > 0;
    });
}
export function requiresCodeScan(body, resolvedRules) {
...
export function validateScanRequestBody(body, resolvedRules) {
