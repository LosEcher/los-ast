import { DEFAULT_EXCERPT_LENGTH, DEFAULT_PARSE_CACHE_MAX_ENTRIES, PARSE_FAILURE_SAMPLE_LIMIT, } from '@los-ast/core';
...
export const SCAN_GOVERNANCE_OVERVIEW = {
    intro: '`/scan` 当前已补齐代码层扫描能力，并支持最小化 `contractArtifacts/schemaArtifacts` 直通，以及 `openApiDocuments/openApiComparisons/schemaDocuments/schemaComparisons` 的原生输入。默认输出的 `findingSource` 为 `ast`，并可与 `contract/schema` findings 并行返回。',
    rows: [
        {
            dimension: '前端/后端接口治理',
    ...
    findingSourceNote: "`findingSource='contract'|'schema'` 是后续演进预留字段，与现有 `findingSource='ast'` 兼容。",
    references: [
...
export const SCAN_VERSION_STABILITY_REFERENCE = {
    ...
    deprecationPolicy: 'Fields may be deprecated with 6-month notice before removal in v2.',
};
export const SCAN_DETERMINISTIC_REFERENCE = {
    ...
    nondeterministicNote: 'When `deterministic: false`, real-time timestamps and full 64-character fingerprints are used.',
};
