...
export function partitionSymbolsByLimit(existingCount, fileSymbols, limit) {
...
export function buildSymbolsFromAstMatches({ matches, kind, file, }) {
    const symbols = [];
...
export function extractSymbolsFromSourceText({ source, file, language, }) {
    const symbols = [];
    const seen = new Set();
...
export function toRange(source, index, length) {
