...
export function buildBreakingFinding(sourceLabel, fileLabel, line, ruleId, message, excerpt) {
    return buildComparisonFinding(sourceLabel, fileLabel, line, ruleId, 'error', message, excerpt, 'high');
}
export function buildComparisonFinding(sourceLabel, fileLabel, line, ruleId, severity, message, excerpt, impactHint) {
    return {
        source: sourceLabel,
        ruleId,
        severity,
...
export function diffEnumValues(baselineValues, currentValues) {
...
export function normalizeUniqueKey(key) {
    return [...key].sort().join('|');
}
export function getSingleFieldUniqueKeys(entity) {
    return new Set((entity.uniqueKeys || [])
        .filter((key) => key.length === 1)
        .map((key) => key[0]));
}
export function isGeneratedDefaultValue(value) {
    return value === '@current_timestamp'
        || value === '@generated_uuid'
        || value === '@generated_increment'
        || value === '@updatedat';
}
export function isLowRiskDefaultRemoval(fieldName, field) {
...
export function isConservativeSqlTypeWidening(baselineType, currentType) {
    const baseline = baselineType.trim().toLowerCase();
    const current = currentType.trim().toLowerCase();
    return ((baseline === 'smallint' && (current === 'integer' || current === 'bigint'))
        || (baseline === 'integer' && current === 'bigint')
        || (baseline === 'real' && current === 'double precision'));
}
export function compareEntities(sourceLabel, fileLabel, prefix, baselineEntities, currentEntities) {
    const artifacts = [];
        ...
        for (const [fieldName, baselineField] of baselineEntity.fields.entries()) {
            ...
            if (baselineField.type !== currentField.type) {
                if (prefix === 'sql' && isConservativeSqlTypeWidening(baselineField.type, currentField.type)) {
                    artifacts.push(buildComparisonFinding(sourceLabel, fileLabel, line, `schema/${prefix}-type-widening`, 'warning', `Field ${baselineEntity.name}.${fieldName} widened type from ${baselineField.type} to ${currentField.type}`, `${baselineEntity.name}.${fieldName}: ${baselineField.type} -> ${currentField.type}`, 'medium'));
                }
                else {
