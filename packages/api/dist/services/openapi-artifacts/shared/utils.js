...
export function getSchemaObject(schema) {
    return isRecord(schema) ? schema : undefined;
}
export function decodeJsonPointerSegment(segment) {
    return segment.replace(/~1/g, '/').replace(/~0/g, '~');
}
export function getLeadingSpaceCount(line) {
    const match = line.match(/^\s*/);
    return match ? match[0].length : 0;
}
export function matchesStructuredKey(trimmedLine, key) {
...
export function parseOperationLabel(operationLabel) {
