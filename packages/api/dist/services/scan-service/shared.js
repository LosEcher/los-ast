...
export function toContractFindingFingerprint(input, deterministic) {
    const base = [
        input.project,
        input.ruleId,
        input.file,
...
export function normalizeGovernanceDomain(value) {
    if (!value) {
        return undefined;
    }
...
export function deterministicSortFindings(a, b) {
    if (a.file !== b.file)
...
export function hasScannableRootDir(rootDir) {
    return typeof rootDir === 'string' && rootDir.trim().length > 0;
}
export function hasNativeArtifactInputs(options) {
    return [
...
export function countNativeInputs(options) {
    return {
...
export function requiresCodeScan(options) {
...
export function buildFindingsFromArtifacts({ project, contractArtifacts, schemaArtifacts, deterministic = false, defaultFindingSource, }) {
...
export function resolveScanMode({ shouldRunAstScan, hasNativeArtifacts, }) {
...
export function buildScanTelemetry({ startedAt, shouldRunAstScan, hasNativeArtifacts, explicitRulePatterns, loadedRules, estimatedFiles, nativeInputCounts, }) {
    return {
...
export function mergeScanResultFindings({ result, contractFindings, schemaFindings, deterministic, }) {
