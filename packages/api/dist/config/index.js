 ...
 * 验证路由前缀格式
 */
function validateRoutePrefix(prefix, name, errors) {
 ...
 * 验证字符串枚举值
 */
function validateEnum(value, validValues, name, errors) {
 ...
 * 验证内部访问控制配置
 */
function validateInternalAccessConfig(errors) {
    ...
    if (IS_PRODUCTION) {
        if (!allowedIps && !token) {
            errors.push('Production safety: INTERNAL_ROUTES_ENABLED=true requires either ' +
 ...
 * 验证环境变量组合
 */
function validateEnvironmentCombinations() {
