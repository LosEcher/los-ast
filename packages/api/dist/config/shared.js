...
const EXPERIMENTAL_STORE_BACKENDS = ['memory', 'file', 'sqlite'];
...
export function resolveNodeEnv(rawNodeEnv) {
    return rawNodeEnv === 'production' || rawNodeEnv === 'test' || rawNodeEnv === 'development'
        ? rawNodeEnv
        : 'development';
}
export function normalizeAndValidateConfig(env) {
    const rawEnv = {
...
export function deriveRuntimeConfig(parsedConfig) {
