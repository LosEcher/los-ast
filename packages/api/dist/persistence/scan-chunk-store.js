...
export class SqliteIntermediateStore {
     ...
     */
    saveChunkResult(chunkId, result) {
        const resultJson = JSON.stringify(result);
        this._database
            .prepare(`INSERT INTO scan_chunks (chunk_id, result_json)
         VALUES (?, ?)
    ...
    loadAllChunkResults() {
        ...
        for (const row of rows) {
            const parsed = parseStoredJson(row.result_json);
            if (parsed) {
                results.push(parsed);
            }
        }
 ...
 */
export function createSqliteIntermediateStore(options) {
