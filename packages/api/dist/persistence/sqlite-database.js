...
const sqliteTransactionDepth = new WeakMap();
export function resolveSqliteDatabasePath(options = {}) {
...
export async function runInSqliteTransaction(callback, options = {}) {
...
export function registerSqliteSchemaVersion(database, schemaName, version) {
    ensureSchemaVersionsTable(database);
...
export function applySqliteMigrations(database, schemaName, migrations) {
    ensureSchemaVersionsTable(database);
...
export function getSqliteSchemaVersion(database, schemaName) {
    ensureSchemaVersionsTable(database);
    const row = database
        .prepare(`SELECT version FROM ${SCHEMA_VERSIONS_TABLE} WHERE schema_name = ?`)
        .get(schemaName);
...
export function closeAllSqliteDatabases() {
...
function ensureSchemaVersionsTable(database) {
    database.exec(`
...
function setSqliteSchemaVersion(database, schemaName, version) {
...
function validateMigrations(schemaName, migrations) {
...
function applyVitestWorkerSuffix(databasePath) {
