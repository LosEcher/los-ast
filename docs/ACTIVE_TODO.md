...
- [x] CLI guard 分析收口：`packages/cli/src/route-guard-analysis/shared.mjs` 已完成第二阶段模块拆分；语法拆分/line mapping 已抽到 `route-guard-analysis/syntax.mjs`，布尔条件归一已抽到 `route-guard-analysis/conditions.mjs`，helper/scope 提取已抽到 `route-guard-analysis/helpers.mjs`，对外 stable re-export 保持不变。
- [x] CLI guard 主文件收口：`packages/cli/src/route-guard-analysis.mjs` 已完成第三阶段模块拆分；control-flow 扫描已抽到 `route-guard-analysis/control-flow.mjs`，guard activation 分类已抽到 `route-guard-analysis/activation.mjs`，route registration 提取已抽到 `route-guard-analysis/registrations.mjs`，并新增 `route-guard-analysis.test.mjs` 作为公共 API 窄回归。
- [x] CLI source structure 收口：`packages/cli/src/source-structure-extractor/shared.mjs` 已完成第二阶段模块拆分；route/path/tier/file-role 分类已抽到 `source-structure-extractor/classification.mjs`，import/reexport/declare 解析已抽到 `source-structure-extractor/parsing.mjs`，static expression / local-module 解析已抽到 `source-structure-extractor/resolution.mjs`，对外 stable re-export 保持不变。
...
- [x] Core 编排收口：`packages/core/src/runner.mjs` 已完成第二阶段模块拆分；discover/scan/fix/explain 编排已分别抽到 `packages/core/src/runner/discover.mjs`、`scan.mjs`、`fix.mjs`、`explain.mjs`，finding/match 记录构造已抽到 `packages/core/src/runner/records.mjs`，并新增 `runner.test.mjs` 作为 public API 窄回归。
...
- 已补单字段唯一约束的语义等价：列级 `UNIQUE` 与表级 `UNIQUE(field)` 不再误报 drift；真实的单字段唯一约束移除会提升为 `warning`。
